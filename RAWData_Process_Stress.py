...
def generate_raw_plots(raw_df, output_dir):
    ...
    try:
        ...
        # Verificar si hay canales para graficar
        if not channels:
            print("Advertencia: No hay canales para graficar en los datos raw")
            return
        ...
        # Guardar la figura
        output_path = os.path.join(output_dir, 'raw_eeg_data.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Gráfico de datos raw guardado en: {output_path}")
        
    except Exception as e:
        print(f"Error al generar gráficos de datos raw: {str(e)}")
    finally:
        # Asegurar que la figura se cierre para liberar memoria
        plt.close('all')
...
def generate_psd_plots(psd_data_dict, bands, output_dir):
    ...
    try:
        ...
        if not psd_data_dict:
            print("Advertencia: No hay datos PSD para graficar")
            return
        ...
        for channel, data in psd_data_dict.items():
            ...
            # Graficar el PSD
            axes[i].semilogy(freqs, psd, linewidth=2)
            axes[i].set_title(f'PSD del Canal {channel}', fontsize=14)
            axes[i].set_ylabel('PSD (uV²/Hz)', fontsize=12)
            axes[i].grid(True, alpha=0.3)
        ...
        axes[-1].set_xlabel('Frecuencia (Hz)', fontsize=12)
        plt.tight_layout()
        
        # Guardar la figura
        output_path = os.path.join(output_dir, 'psd_by_channel.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Gráfico de PSD guardado en: {output_path}")
        
    except Exception as e:
        print(f"Error al generar gráficos de PSD: {str(e)}")
    finally:
        # Asegurar que la figura se cierre para liberar memoria
        plt.close('all')
...
def generate_stress_plots(results_df, stress_analysis, output_dir):
    ...
    try:
        # Verificar si hay datos para graficar
        if results_df.empty:
            print("Advertencia: No hay datos de estrés para graficar")
            return
        ...
        ax = sns.barplot(x='Channel', y='Stress_Ratio', data=results_df, palette=colors)
        ...
        plt.title('Ratio de Estrés por Canal (Theta/Beta)\nValores más bajos indican mayor estrés', fontsize=14)
        plt.ylabel('Ratio Theta/Beta', fontsize=12)
        ...
        # Guardar la figura
        output_path = os.path.join(output_dir, 'stress_ratio_by_channel.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Gráfico de ratio de estrés guardado en: {output_path}")
        plt.close()
        ...
        if not available_cols:
            print("Advertencia: No hay métricas disponibles para el heatmap")
            return
            
        # Crear un DataFrame pivoteado para el heatmap
        heatmap_df = results_df.pivot(index='Channel', columns=None, values=available_cols)
        ...
        sns.heatmap(normalized_df, annot=True, fmt=".2f", cmap="coolwarm", center=0, 
                   annot_kws={"size": 10}, cbar_kws={'label': 'Valor Normalizado'})
        
        plt.title('Mapa de Calor de Métricas EEG por Canal (Valores Normalizados)', fontsize=14)
        plt.tight_layout()
        
        # Guardar la figura
        output_path = os.path.join(output_dir, 'eeg_metrics_heatmap.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        print(f"Heatmap de métricas EEG guardado en: {output_path}")
        
    except Exception as e:
        print(f"Error al generar gráficos de estrés: {str(e)}")
    finally:
        # Asegurar que la figura se cierre para liberar memoria
        plt.close('all')
...
def main(input_file):
    ...
    if stress_analysis['overall_stress_level'] == "ALTO":
        print("\n ALERTA: ¡NIVEL DE ESTRÉS ELEVADO DETECTADO! ")
